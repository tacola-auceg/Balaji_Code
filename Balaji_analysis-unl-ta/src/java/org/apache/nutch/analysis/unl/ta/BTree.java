...
public class BTree extends AbstractSet implements Set
{
	...
	public void updateDictTree(String fileName)
	{
		...
		try
		{
		      // JarFile jarFile = new JarFile("./resource/unl/Dictionary.jar");
		       //JarEntry entry = jarFile.getJarEntry(fileName);
		       //InputStream input1 = jarFile.getInputStream(entry);
			File f=new File(fileName);
			FileInputStream input1 =new FileInputStream(f);
			//////System.out.println(fileName);
		       // dic = new BufferedReader(new InputStreamReader(input1,"UTF8"));
			BufferedReader bis=new BufferedReader(new InputStreamReader(input1,"UTF8"));
			StreamTokenizer input = new StreamTokenizer(bis);
			int tokentype = 0;
			...
			while ((tokentype = input.nextToken()) != StreamTokenizer.TT_EOF)
			{
				...
				if (tokentype == StreamTokenizer.TT_WORD)
				{
					...
					byte b[]=UnicodeConverter.convert(input.sval);
...
//	}
		...
		private Object nextElement()
		{
			if (currentNode.isLeaf()) // if leaf
			{
				...
				} else if (!parentIndex.empty())
				{
					...
					if (index == currentNode.nrElements)
					{ // reached root and no more children
						return null;
					}
	...
	private class BTreeNode
	{
		...
		private BTreeNode split()
		{
			...
			for (int i = 0, nr = nrElements; index <= nr; i++, index++)
			{
				...
				if (rightSibling.entries[i] != null
				  && rightSibling.entries[i].child != null)
				{
		...
		private int childToInsertAt(Object Obj, boolean position)
		{
			...
			return (!position
			  ? max
			  : BTree.this.compare(Obj1, Obj2) == 0 ? -1 : max);
		}
	...
	public boolean equals(byte[] input)
	{
		Iterator iterator = new Iterator();
		//////System.out.println("checking	:"+UnicodeConverter.revert(input));
		for (int i = 0; i < size; i++)
		{
			String s = ((String) iterator.next()).intern();
			byte[] b = UnicodeConverter.convert(s);

			if (ByteMeth.isEqual(input, b))
			{
				//////System.out.println("word1	:"+UnicodeConverter.revert(input)+"	word2:"+UnicodeConverter.revert(b));
				return true;
			}
